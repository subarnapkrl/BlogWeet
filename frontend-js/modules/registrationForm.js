...
export default class RegistrationForm{
    ...
    events(){
        ...
        this.password.addEventListener("keyup",()=>{
            this.isDifferent(this.password,this.passwordlHandler)
        })
    ...
    passwordAfterDelay()
    {
        if(this.password.value.length<6){
            this.showValidationError(this.password,"Password must be at least 6 chaaracters.")
        }
    }
    ...
    emailAfterDelay()
    {
        ...
        if(!this.email.errors){
            axios.post('/doesEmailExist',{email:this.email.value}).then((response)=>{
                ...
                ConnectionClosedEvent.log("Please Try Again Later.")
            })
        }
    }
